...
namespace ReceiptStorage.View
{
    public partial class Location : PhoneApplicationPage
    {
        ...
        private GeoCoordinate shopLocation;
        private string nokiaPlacesHereUrl = SettingsApi.NokiaPlacesHereUrl;
        ...
        protected override void OnNavigatedTo(System.Windows.Navigation.NavigationEventArgs e)
        {
            ...
            if (NavigationContext.QueryString.TryGetValue("position", out position) && NavigationContext.QueryString.TryGetValue("placeName", out placeName))
            {
                var tmp = position.Split(',');
                shopLocation = new GeoCoordinate();

                shopLocation.Latitude = double.Parse(tmp[0], CultureInfo.InvariantCulture);
                shopLocation.Longitude =double.Parse(tmp[1], CultureInfo.InvariantCulture);

                Pushpin pushpin = new Pushpin();
                pushpin.Background = new SolidColorBrush(Colors.Black);
                pushpin.Opacity = 1.0;
                pushpin.Location = shopLocation;
                ...
                mapPlaces.SetView(shopLocation, 17);
